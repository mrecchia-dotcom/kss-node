...
const cli = function(opts) {
  ...
  const cliOptionDefinitions = {
    'config': {
      ...
      configParser: (configPath) => {
        ...
        config.base = config.base ? path.resolve(configDirectory, config.base) : configDirectory;

        config.homepage = path.resolve(config.base, config.homepage || 'homepage.md');

        if (config.source) {
          if (Array.isArray(config.source)) {
            config.source = config.source.map(x => path.resolve(config.base, x));
          } else {
  ...
  return checkBuilderPath.then(() => {
    ...
    options = yargs(args)
      ...
      // Disable OS locale auto-detection and force English so help/error
      // text is consistent regardless of the system's locale (e.g.
      // LANG=fr_FR.UTF-8).
      .detectLocale(false)
      .locale('en')
